...
class Visualizer:
    def __init__(self,
        ...
        if draw_plane:
            self._draw_plane(world_center=np.array([0, 0, 0]), grid_size=50, tile_size=1)
        if is_record:
    ...
    def _draw_plane(self, world_center: np.ndarray, grid_size: int, tile_size: int = 1) -> None:
        ...
        plane = pv.Plane(
            ...
            direction=(0, 0, 1),  # 법선 벡터 (xz 평면 생성)
    ...
    # def set_camera_position(self, world_pose):
    #     # 카메라 중심 위치
    #     cam_center = world_pose[:3, 3]
    ...
    #     offset_distance_z = 15.0  # 카메라 뒤쪽으로의 거리
    ...
    def set_camera_position(self, world_pose):
        cam_center = world_pose[:3, 3]       # 카메라 중심 위치
        cam_forward = world_pose[:3, 2]      # 카메라가 향하는 방향 (Z축)
        cam_up = world_pose[:3, 1]           # 위쪽 방향 (Y축)

        offset_z = 5.0  # 뒤로 떨어진 거리
        offset_y = 1.5  # 약간 위에서 봄

        camera_position = cam_center - cam_forward * offset_z + cam_up * offset_y

        focal_point = cam_center + cam_forward * 3.0

        # 위쪽 방향은 고정된 world 기준 Y축
