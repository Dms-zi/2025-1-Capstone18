...
def basic_block_down(inputs, num_channels, kernel_size, name):
    ...
    x = tf_keras.layers.Conv2D(num_channels, kernel_size=1, strides=2, padding='same', activation='linear', use_bias=False, name=name+'.0.downsample.0')(x)
    x = tf_keras.layers.BatchNormalization(momentum=BATCH_NORM_DECAY, epsilon=1e-5, name=name+'.0.downsample.1')(x)
    x = tf_keras.layers.Add()([x, x1])
    x = tf_keras.layers.Activation('relu')(x)
    return x
...
def conv_bn_relu(x, num_channels, kernel_size, strides, name):
    ...
    if strides[0] == 2:
        x = tf_keras.layers.ZeroPadding2D(padding=(1,1), name=name+'.pad')(x)
        x = tf_keras.layers.Conv2D(num_channels, kernel_size, strides[0], padding='valid', activation='linear', use_bias=False, name=name+'.conv1')(x)
    else:
        x = tf_keras.layers.Conv2D(num_channels, kernel_size, strides[0], padding='same', activation='linear',  use_bias=False, name=name+'.conv1')(x)      
    x = tf_keras.layers.BatchNormalization(momentum=BATCH_NORM_DECAY, epsilon=1e-5, name=name+'.bn1')(x)
    x = tf_keras.layers.Activation('relu')(x)

    x = tf_keras.layers.Conv2D(num_channels, kernel_size, strides[1], padding='same', activation='linear', use_bias=False, name=name+'.conv2')(x)
    x = tf_keras.layers.BatchNormalization(momentum=BATCH_NORM_DECAY, epsilon=1e-5, name=name+'.bn2')(x)
    return x