...
class MarsLoggerHandler(object):
    def __init__(self, config):
        self.config = config
        self.root_dir = '../vo/Silsub_0425'
        #'/workspace/Deep-Visual-SLAM-main/vo/Silsub_0425' #'/media/park-ubuntu/park_cs/slam_data/mars_logger'
        self.image_size = (self.config['Train']['img_h'], self.config['Train']['img_w'])
        self.num_source = self.config['Train']['num_source'] # 1 양쪽에 1장씩 할것
        #self.imu_seq_len = self.config['Train']['imu_seq_len'] # 10 imu, 사용안함
        ...
        self.test_dir = '../vo/last/'#os.path.join(self.root_dir, 'test')
    ...
    #     dataset = glob.glob(os.path.join(rgb_save_path, '*.jpg'))
        dataset = sorted(glob.glob(os.path.join(scene_dir, '*.jpg')))
        ...
        original_image_size = (1920,1080)#(camera_metadata['image_height'], camera_metadata['image_width'])
        
        intrinsic = np.array([[1.79214807e+03, 0.00000000e+00, 5.31603784e+02],
                                [0.00000000e+00, 1.79119403e+03, 9.72880606e+02],
                                [0.00000000e+00, 0.00000000e+00, 1.00000000e+00]]) #np.load(os.path.join(camera_metadata_path, 'camera_matrix.npy'))
    ...
    def _process(self, scene_dir: str, camera_name: str, is_test: bool=False) -> list:
        ...
        rgb_files = sorted(glob.glob(os.path.join(scene_dir, '*.jpg')))#sorted(glob.glob(os.path.join(scene_dir, 'rgb', '*.jpg')))
    ...
    def generate_test(self, test_dir):
        ...
        datasets = np.concatenate(datasets, axis=0)
        print('generate_test')
        print(f' dadtasets are: {datasets}')
        return datasets
