...
class CustomDataHandler(object):
    def __init__(self, config):
        self.config = config
        self.root_dir = '/workspace/Deep-Visual-SLAM-main/' #'/media/park-ubuntu/park_cs/custom_dataset'
    ...
    def generate_datasets(self, fold_dir, shuffle=False):
        ...
        with open(os.path.join(fold_dir, 'scene.txt'), 'r') as f:
            scene_files = f.readlines()
        scene_files = [os.path.join(fold_dir, line.strip()) for line in scene_files]
